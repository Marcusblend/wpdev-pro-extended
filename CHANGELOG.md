...
- **`create_snapshot` and `restore_snapshot`**: one call captures what makes a site look like itself — palette, fonts, Global CSS, the Theme Options that differ from their defaults, Global Variables, global parameters, menus with their items and locations, and an inventory of the documents — and another puts the writable parts back, backing up each option first and showing the diff until `confirm: true`. Documents are listed rather than captured: their element data belongs in `backup_layout` and `export_tco`, and carrying it here would bury the settings. Keys another part already owns are left out of the Theme Options, so the two halves cannot restore over each other, and an option that was never set is not reported as a change against an empty snapshot value
